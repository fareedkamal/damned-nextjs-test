...
export async function POST(request: Request) {
  try {
    ...
    if (!input.customerId && !input.billing?.email) {
      return NextResponse.json({ errors: { message: 'Missing user identifiers' } }, { status: 500 });
    }

    const client = getClient();
    if (!process.env.CREATE_ORDER_PASSWORD) {
      return NextResponse.json({ errors: { message: 'System credentials missing. Please contact the administrator.' } }, { status: 500 });
    }
    ...
    if (!createOrder?.order) {
      return NextResponse.json({ errors: { message: 'Failed to create order' } }, { status: 500 });
    }
